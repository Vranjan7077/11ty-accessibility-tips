...
- **Hover-only tooltips** - Must also appear on keyboard focus. This is really an instance of a wider problem - see [device dependent event handlers](/topics/javascript/device-dependent-event-handlers/) for why hover-only interactions leave keyboard and touch users out entirely.
...
- **Trigger too small to tap reliably** - See [touch target size and spacing](/topics/css/touch-target-size-and-spacing/) for the minimum size a tooltip trigger needs on a touchscreen.
