...
> **Note:** No major browser actually speaks `<track kind="descriptions">` content aloud - browser support for synthesizing narration from a WebVTT descriptions track is effectively nonexistent. The track is still useful for exposing description text programmatically, but for real audio description, produce an extended/open-described version of the video itself or use a third-party audio-description service.
...
Autoplay videos should be paused for users who prefer reduced motion - see [reduced motion and animations](/topics/css/reduced-motion-and-animations/) for the full pattern:
...
-   **Slider controls** - Volume and seek bars should use `role="slider"` with `aria-valuemin`, `aria-valuemax`, `aria-valuenow`, and `aria-valuetext` - see [ARIA roles, states, and properties](/topics/aria/aria-roles-states-and-properties/) for what each of these actually communicates to a screen reader.
